    ...
    <MyModal modalVisible={modalVisible} closeModal={closeModal} type={modalType}>
      <div>
        <div className="airdrop_connection_status_title">
          {droppedAirdrops == 0 ? "All bottles online" : "Bottles that have lost connection"}
        </div>
        {droppedAirdrops == 0 ? (
          <img src={sunglass_emoji} alt="Sunglasses Emoji" width={"368px"} height={"270px"} />
        ) : null}
            ...
            ? obcStatus.droppedAirdropIdx.map((airdrop, index) => {
                return (
                  <div key={index}>
                    ...
                    />
                    <div className="airdrop_connection_status_airdrop_number">{airdrop}</div>
                  </div>
                );
              })
            : null}

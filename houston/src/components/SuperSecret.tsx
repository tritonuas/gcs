...
const ALL_STORE_ITEMS = [
  new StoreItem(
    tuasMember,
    "TUAS Member",
    "A TUAS member who will help by making additional orders.",
    20,
    0.1,
  ),
  ...
  new StoreItem(
    robot,
    "Robot",
    "A TUAS developed autonomous robot which will make additional orders",
    2000,
    25,
  ),
  new StoreItem(
    printer,
    "Printer",
    "A TUAS developed black-market Tapex receipt printer",
    10000,
    100,
  ),
];
...
export function SuperSecret() {
  ...
  const [currDescription, setCurrDescription] = useState(
    `Receipts per second ${calculateReceiptsPerSecond()}`,
  );
  ...
  function addItem(index: number) {
    setInventory(
      inventory.map((numCurrItem, currIndex) => {
        if (index == currIndex) {
  ...
  return (
    <div className={"map s-s"}>
      {started ? (
        ...
        <h1 className={"title"}>TUAS Clicker</h1>
      )}
            ...
            return (
              ...
              >
                <img
                  ...
                  onMouseLeave={() => {
                    setCurrDescription(
                      `Receipts per second: ${roundDecimal(calculateReceiptsPerSecond())}`,
                    );
                  }}
                />
                <p>{item.name}</p>
      ...
      <div className="description">{started ? currDescription : ""}</div>
    </div>
  );
}

...
function GetNextColor() {
  return [0, (Math.random() * 255) % 255, (Math.random() * 255) % 255];
}
// --- Component ---
const Reports: React.FC = () => {
  ...
  /**
   * Updates the detections state with new data fetched from the backend, which fetches new runs from the OBC
   */
  function updateClusters() {
    fetch(TARGETS_ALL_ENDPOINT).catch((e) => {
      ...
      .then((j) => {
        ...
        for (const [key, value] of Object.entries(j)) {
          ...
          for (const d of (value as { detections: { location: GPSCoord; image: string, rejected : boolean, id: number }[] })[
            "detections"
          ]) {
  ...
  function syncWithoutFetchingOBC() {
    fetch(FETCH_CLUSTERS_ENDPOINT)
      ...
      .then((j) => {
        console.log("Data obtained from go proxy without obc update:", j);
        //Later, this would make sense to us a protobuffer for, once the format is more set
        const newval = [];
        // for now, overrid e the entire thing. Maybe latter someone can change this to only send new ones, but bandwidth isn't an issue since this should only ever happen across local points
        for (const [key, value] of Object.entries(j)) {
  ...
  function toggleRejectionStatus(id: number) {
    ...
    fetch(`${TOGGLE_ENDPOINT}?id=${id}`)
      .catch((e) => {
        window.alert("failed to toggle: " + e );
      })
      .then(() => {

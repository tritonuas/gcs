 ...
 */
    ...
    return <>
      {
        cluster.all_data_points.map((e, i) => {
          ...
          return <Circle key = {i} eventHandlers={{
            click: () => {
              setMapLoc([e.location.Latitude, e.location.Longitude])
              setSelectedDetection(e);
            }
          }} center={[e.location.Latitude, e.location.Longitude]} radius={e == selectedDetection ? 200 : 100} pathOptions={{
            color: e.rejected ? 'red' : `rgb(${cluster.color[0]}, ${cluster.color[1]}, ${cluster.color[2]})`
          }} />
        })
      }
      <Marker
        ...
        <Popup>
          Cluster for airdrop: {cluster.airdrop_type}
        </Popup>
  ...
  return <main className="reports-main">
        ...
        <div className="reports-cluster-data">
          ...
          <div className="reports-cluster-bottom-section">{
            ...
            <div className="reports-cluster-container">
              <div>Airdrop {selectedCluster?.airdrop_type}
                <br></br>
                Current Chosen Center: {selectedCluster && GPSCoordToString(selectedCluster.selected_center ?? selectedCluster.calculated_center)}
                <br></br>
                Calculated Center: {selectedCluster && GPSCoordToString(selectedCluster.calculated_center)}
              </div>
              <div className="reports-table-wrapper">
                <table className="reports-cluster-table">
                  ...
                  selectedCluster.all_data_points.map((p,i) => {
                      return <tr key = {i} className="reports-cluster-table-row" onClick={() => {
                        ...
                        <td>
                          {GPSCoordToString(p.location)}
                        </td>
                        <td>
                          {p.rejected ? <span style={{color: "red"}}>False</span> : <span style={{color: "green"}}>True</span>}
                        </td>
                      </tr>
                    })
                  }
    ...
    <div className="reports-col" >
      <div className="reports-card report-detection-card">
        {
          selectedDetection ? <div className="report-detection-container">
            ...
            <div>
              ...
              <div>
                <button onClick={
                  ...
                  {selectedDetection.rejected ?  <span style={{color: "green"}}>Re-accept point</span> : <span style={{color: "red"}}>Reject Point</span> }
                </button>
              </div>
            </div>
          </div> : <h1>Select a detection on the map or  from the table to view</h1>
        }
      </div>
      <div className="reports-card">Temp dev stuff idk what goes here yets <br></br>
        <button onClick={() => {
          const center = GPSCoord.create({ Latitude: maploc[0] + Math.random() * 0.02 - 0.01, Longitude: maploc[1] + Math.random() * 0.02 - 0.01, Altitude: 0 });

...
function Image({ item, matchedItems }: ImageProps) {
    ...
    const matchIndex = matchedItems.findIndex(
        (itemX) => itemX.Target?.id === item.id
    );
    ...
    return match ? (
        ...
        <div className="image-container">
            ...
            <p className="image-data">
                <b>Objects:</b> {oDLCObjectsToJSON(item.Object)}{" "}
            </p>
            <Button
                className="button"
                size="small"
...
function BottleImage({ item, matchedItems }: BottleProps) {
    const matchIndex = matchedItems.findIndex((itemX) =>
        itemX.Bottle ? itemX.Bottle.Index === item.Index : null
    );
    let backgroundColor = { backgroundColor: "grey" };
    if (matchIndex !== -1) {
        backgroundColor = { backgroundColor: button_colors[matchIndex] };
    ...
    return (
        <div className="image-container" style={backgroundColor}>
            ...
            <p className="image-data">
                <b>Target ID:</b> {matchedItems[matchIndex].Target?.id}
            </p>
        </div>
    );
}
...
function Report() {
    ...
    const matchedStyle = matched
        ...
        : {
              ...
              boxShadow: "0 0 10px 0 rgba(0, 0, 0, 0.75) inset",
          };
    const unmatchedStyle = unmatched
        ? { backgroundColor: "var(--highlight)" }
        : {
              backgroundColor: "#808080",
              boxShadow: "0 0 10px 0 rgba(0, 0, 0, 0.75) inset",
    ...
    const unmatchedArray = unmatched
        ? foundItemArray.filter(
              (item) =>
                  !itemArray.find(
                      (matchedItem) => matchedItem.Target?.id === item.id
                  )
          )
        : [];

    const unmatchedIcons = unmatched
        ? unmatchedArray.map((item) =>
              L.icon({
            ...
            <div className="left-container">
                <div className="gallery-container">
                    <div className="unmatched-gallery">
                        {foundItemArray.map((item, i) => (
                            <Image
                                key={i}
                                item={item}
                                matchedItems={itemArray}
                                foundItemIndex={foundItemArray.indexOf(item)}
                                updateMatched={updateMatched}
                            />
                        ))}
                    </div>
                ...
                <TuasMap className={"report-page-map"} lat={lat} lng={lng}>
                    {matchedArray.map((marker) => (
                        <Marker
                            ...
                            position={[
                                marker.Target!.coordinate?.Latitude || lat,
                                marker.Target!.coordinate?.Longitude || lng,
                            ]}
                            icon={matchedIcons[matchedArray.indexOf(marker)]}
                        >
                            <Popup>
                                {" "}
                                {oDLCObjectsToJSON(marker.Target!.Object)}
                            </Popup>
                        </Marker>
                    ))}
                    {unmatchedArray.map((marker) => (
                        <Marker
                            ...
                            icon={
                                unmatchedIcons[unmatchedArray.indexOf(marker)]
                            }
                        >

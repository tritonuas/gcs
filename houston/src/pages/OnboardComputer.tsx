...
function OnboardComputer() {
  ...
  const [obcStatus, setOBCStatus] = useState<OBCConnInfo>(
    JSON.parse(localStorage.getItem("obc_conn_status") || "{}") as OBCConnInfo,
  );
  ...
  return (
    <>
      <PageOpenPopup storageKey="ubiquiti-popup" contentLabel="Ubiquiti Login Information">
        <p>
      ...
      <main className="obc-page">
        <ul className="status-list">
          ...
          <li>cameraGood: {obcStatus.cameraGood ? "true" : "false"}</li>
          <table>
            <tr>
              <th>Airdrop Idx</th>
              <th>MS Since HB</th>
            ...
            {"droppedAirdropIdx" in obcStatus ? (
              obcStatus.droppedAirdropIdx.map((_, i) => {
                return (
                  <tr key={i}>
                    <td>{obcStatus.droppedAirdropIdx[i]}</td>
                    <td>{obcStatus.msSinceAdHeartbeat[i]}</td>
                  </tr>
                );
              })
            ) : (
          ...
          onClick={() => {
            fetch("/api/camera/capture")
              ...
              .then((json) => {
                ...
                setMetadatas((metadatas) => [...metadatas, { lat: msg.latDeg, lng: msg.lngDeg }]);
              })
              .catch((err) => alert(err));
          }}
        >
              ...
              .then((resp) => {
                if (!resp.ok) {
                  return resp.text().then((text) => {
                    throw new Error(
                      `Airdrop command failed: ${text || resp.statusText} (Status: ${resp.status})`,
                    );
                  });
                }
                return resp.text();
        ...
        <button
          type="button"
          onClick={() => {
            const valueToSend = intervalMS <= 0 ? DEFAULT_INTERVAL_MS : intervalMS;
            fetch("/api/camera/startstream", {
              ...
              .then((responseText) => {
                alert(responseText || "Camera stream start command successfully processed!");
              })
              .catch((err) => {
                console.error("Camera stream start error:", err);
                alert(err.message || "An unexpected error occurred during camera stream start.");
              });
          }}
        >
          Start camera stream
        ...
        <button
          ...
          onClick={() => {
            fetch("/api/camera/endstream", {
              ...
              .then((responseText) => {
                alert(responseText || "Camera stream end command successfully processed!");
              })
              .catch((err) => {
                console.error("Camera stream end error:", err);
                alert(err.message || "An unexpected error occurred during camera stream end.");
              });
          }}
        >
          End camera stream
            ...
            fetch("/api/camera/runpipeline", {
              ...
              .then((resp) => {
                if (!resp.ok) {
                  return resp.text().then((text) => {
                    throw new Error(
                      `Pipeline run failed: ${text || resp.statusText} (Status: ${resp.status})`,
                    );
                  });
                }
                return resp.text();
              ...
              .then((responseText) => {
                alert(responseText || "Run pipeline command successfully processed!");
              })
              .catch((err) => {
                console.error("Run pipeline end error:", err);
                alert(err.message || "Run pipeline error");
              });
          }}
        >
          Run Pipeline
        ...
        <div className="image-container">
          <div className="gallery-container">
            <ImageGallery onSlide={handleSlide} items={pictures}></ImageGallery>
          </div>
          <TuasMap className={"map"} lat={selectedLat} lng={selectedLng}>

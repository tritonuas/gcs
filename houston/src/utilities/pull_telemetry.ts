...
export function pullTelemetry(
  ...
  fetch("/api/plane/voltage")
    ...
    .then((json) => {
      ...
      setPixhawkBatteryVal((param) =>
        param.getUpdatedValue(pixhawkV / PIXHAWK_CELLS, (x) => x * PIXHAWK_CELLS),
      );
      setMotorBatteryVal((param) =>
        param.getUpdatedValue(motorV / PIXHAWK_CELLS, (x) => x * MOTOR_CELLS),
      );
    })
    .catch((_) => {

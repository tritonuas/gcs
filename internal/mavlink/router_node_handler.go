...
// responsible for managing the other nodes that we are forwarding/receiving nodes to/from
// only needed for the mavlink router that is included in hub
const (
		...
		func() {
			...
			for rnode, t := range nh.remoteNodes {
				if now.Sub(t) >= nodeInactiveAfter {
					Log.Infof("node disappeared: %s", rnode)
...
func (nh *nodeHandler) onEventFrame(evt *gomavlib.EventFrame) {
	...
	if _, ok := nh.remoteNodes[rnode]; !ok {
		Log.Infof("node appeared: %s", rnode)
	}
